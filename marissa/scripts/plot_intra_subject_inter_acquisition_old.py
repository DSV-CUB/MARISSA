...
for i in range(len(sys_seq)):
    ...
    legend_elements.append(Line2D([0], [0], color=rgba, lw=2, label=sys_seq[i].replace("#", " | ").replace(" Healthcare ", " ").replace(" Medical Systems ", " ")))


#ax[-1, -1].legend(handles=legend_elements, loc='upper left', frameon=False, fontsize=20)
#ax[-1, -1].axis("off")
axbig.legend(handles=legend_elements, loc='upper left', frameon=False, fontsize=20)
axbig.axis("off")
