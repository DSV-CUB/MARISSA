...
def plot_roc(ax, roc_analysis, c="blue", optimum_colour="#ff0000", optimum_text=True, label="", zorder=10):
    if c=="C3":
        c="C9"
    if optimum_colour == "C3":
        optimum_colour="C9"

    ax.plot(roc_analysis["roc_fpr"] * 100, roc_analysis["roc_tpr"] * 100, c=c, label=label, zorder=zorder)
...
fig = plt.figure(None, figsize=(15, 10), constrained_layout=True)
...
ax = np.vstack((axes_sf0, axes_sf1))


subfigs[0].supylabel("before standardization", fontsize=12, fontweight="bold")
subfigs[1].supylabel("after standardization", fontsize=12, fontweight="bold")
ax[0,0].set_title("Healthy vs. HCM", fontsize=12, fontweight="bold")
ax[0,1].set_title("Healthy vs. Amyloidosis", fontsize=12, fontweight="bold")
ax[0,2].set_title("HCM vs. Amyloidosis", fontsize=12, fontweight="bold")
...
for uamy in np.unique(parameter_AMY):
    ...
    if len(indeces) > 0:
        ...
        plot_roc(ax[0,1], roc_analysis, c=color, optimum_colour=color, optimum_text=False,  label=uamy + "\nsensitivity = " + "{:.2f}".format(np.round(roc_analysis["optimal_roc_point"][1] * 100, 2)) + "% | specificity = " + "{:.2f}".format(np.round(100 - roc_analysis["optimal_roc_point"][0] * 100, 2)) + " %", zorder=10)
...
plot_roc(ax[1,0], roc_analysis_healthy_hcm, c="C0", optimum_colour="#ff0000", optimum_text=True,  label="", zorder=10)
plot_roc(ax[1,1], roc_analysis_healthy_amy, c="C0", optimum_colour="#ff0000", optimum_text=True,  label="", zorder=10)
plot_roc(ax[1,2], roc_analysis_hcm_amy, c="C0", optimum_colour="#ff0000", optimum_text=True,  label="", zorder=10)

ax[0,0].legend(loc="lower right", fontsize=7)
ax[0,1].legend(loc="lower right", fontsize=7)
ax[0,2].legend(loc="lower right", fontsize=7)
